package com.ydky.vuandroidadsdk.widget;
...
import com.ydky.vuandroidadsdk.adutil.LogUtils;
...
//    private static final int STATE_PAUSING = 2;
//
//    private static final int LOAD_TOTAL_COUNT = 3;  //视频首次加载失败后重新加载次数
//
//    /**
//     * UI
//     */
...
//
//    private MediaPlayer mediaPlayer;
//    private ADVideoPlayerListener listener;  //事件回调监听
//    private ScreenEventReceiver mScreenReceiver;  //监听屏幕是否锁屏
//    private Handler mHandler = new Handler(Looper.getMainLooper()) {  //每隔1秒发送一个time_msg给服务器，通知当前播放进度，不推荐使用timer，容易造成内存泄露
//        @Override
//        public void handleMessage(Message msg) {
//            switch (msg.what) {
//                case TIME_MSG:
...
//    }
//
//    //触摸事件处理，直接返回true,表明只要触摸事件到了我们的播放控件中，就把事件消耗掉，防止与父容器产生时间冲突
//    @Override
...
//            mute(true);
//            mediaPlayer.setDataSource(this.mUrl);
//            mediaPlayer.prepareAsync(); //开始异步加载视频
//        } catch (Exception e) {
//            LogUtils.e(TAG, e.getMessage());
//            stop(); //error以后重新调用stop加载
//        }
...
//    }
//
//    //seekAndResume和seekAndPause是在处理小屏和大屏切换播放后，会从切换前的位置继续播放
//    /**
//     * 跳到指定点播放视频
//     * @param position
//     */
...
//        if (isPlaying()) {
//            mediaPlayer.seekTo(position);
////            mediaPlayer.pause();   //这种实现方式只能适配部分手机，机型不确定，下面回调的方式，适配所有手机
//             //seekTo的事件监听
//            mediaPlayer.setOnSeekCompleteListener(new MediaPlayer.OnSeekCompleteListener() {
//                @Override
//                public void onSeekComplete(MediaPlayer mp) {
